...
namespace FlappyBirdXD
{
	CreditScreen::CreditScreen()
	{
		...
		returnButton = new Button({ GetScreenWidth() * 0.5f, GetScreenHeight() - 50.0f }, "Volver", clickSfxUrl.c_str(), static_cast<int>(GetScreenHeight() * 0.05f), 20);
	...
	{
		DrawText("Creditos", static_cast<int>(GetScreenWidth() * 0.5f - (MeasureText("Creditos", static_cast<int>(GetScreenHeight() * 0.125f)) * 0.5f)), 0, static_cast<int>(GetScreenHeight() * 0.125f), BLACK);
		myLinkedin->draw();
